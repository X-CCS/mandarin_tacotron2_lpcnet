	...
	def synthesize(self, texts, basenames, out_dir, log_dir, mel_filenames):
		...
		for i, mel in enumerate(mels):
			...
			if log_dir is not None:
				...
				#save alignments
				plot.plot_alignment(alignments[i], os.path.join(log_dir, 'plots/alignment-{:03d}.png'.format(basenames[i])),info='{}'.format(texts[i]), split_title=True)

				#save mel spectrogram plot
				plot.plot_spectrogram(mel, os.path.join(log_dir, 'plots/mel-{:03d}.png'.format(basenames[i])),info='{}'.format(texts[i]), split_title=True)
				...
				if hparams.predict_linear and not self.gta:
					...
					#save mel spectrogram plot
					plot.plot_spectrogram(linears[i], os.path.join(log_dir, 'plots/linear-{}.png'.format(basenames[i])),info='{}'.format(texts[i]), split_title=True, auto_aspect=True)
